  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      >
        ...
        {evolutionData.secondStage.length > 0 && <FaArrowDown className="text-2xl" />}
          ...
          {evolutionData.secondStage.length > 0 &&
            evolutionData.secondStage.map((item, index) => (
              <li key={index} className="text-lg capitalize text-center p-3">
                {/* <span>{item.trigger.split("-").join(" ")}</span> */}
                <PokemonCard name={item.name} />
              </li>
            ))}
        </ul>
        {evolutionData.thirdStage.length > 0 && <FaArrowDown className="text-2xl" />}
        <ul className="flex gap-4 overflow-x-auto">
